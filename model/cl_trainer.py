...
class ClassifierTrainer(object):
    ...
    def run_step(self, review, optimizer, keys, moviereview, labels, valid=False):
        if not valid:
            optimizer.zero_grad()
        logits_xw, logits_w, probs_xw, probs_w = \
            review.classifier_graph(keys, moviereview, self.__factor_id)
        ...
        if not valid:
            loss.backward()
            clip_grad_norm_(review.cl_parameters(self.__factor_id),
    ...
    def run_train(self, review, tool, optimizer, logger):
        ...
        for step in range(0, tool.train_batch_num):
            ...
            batch_keys = batch[0].to(device)
            batch_lines = batch[1].to(device)
            batch_labels = batch[2].to(device)

            loss_xw, loss_w, preds_xw, preds_w = \
                self.run_step(review, optimizer,
                    batch_keys, batch_lines, batch_labels)
    ...
    def train(self, review, tool, factor_id):
        self.__factor_id = factor_id

        utils.print_parameter_list(review, review.classifier_parameter_names(factor_id))
        #input("please check the parameters, and then press any key to continue >")
        # 加载数据进行预训练
        print("为分类器构建数据, factor_id: %d ..." % (factor_id))
        ...
        # 构建优化器
        opt = torch.optim.AdamW(review.cl_parameters(self.__factor_id),
        ...
        for epoch in range(1, self.hps.cl_epoches+1):
            ...
            if (self.hps.cl_save_epoches >= 0) and \
                ...
                # save checkpoint
                print("保存模型...")
                utils.save_checkpoint(self.hps.model_dir, epoch, review,
