...
class Tool(object):
    ...
    def __init__(self, sens_num, key_len,
        sen_len, po_len, corrupt_ratio=0):
    ...
    def build_data(self, train_data_path, valid_data_path, batch_size, mode):
        ...
        NOTE: 先运行load_dic()
        mode:
            cl1 or cl2: 预训练因子1或预训练因子2的分类器
            dae: 将编码器和解码器预训练为去噪自动编码器
            review_pre: 使用标记和未标记数据训练 review
            review_tune: 仅使用标记数据微调 review
        '''
        assert mode in ['cl1', 'cl2', 'dae', 'review_pre', 'review_tune']
    ...
    def __build_data_core(self, data, batch_size, mode, data_limit=None):
        ...
        elif mode == 'review_pre' or mode == 'review_tune':
            return self.build_review_batches(data, batch_size, mode)
    ...
    def build_classifier_batches(self, ori_data, batch_size, mode):
        ...
        for bi in range(0, batch_num):
            ...
            reviews = [instance[1] for instance in instances] # all reviews
    ...
    def build_dae_batches(self, data, batch_size):
        ...
        for bi in range(0, batch_num):
            ...
            # build review batch
            reviews = [instance[1] for instance in instances] # all reviews
            # sequences = [sum(rev, []) for rev in reviews]

            batch_reviews = self.__build_batch_seqs(reviews, True, True, corrupt=True)
            ...
            batch_lines = self.__build_batch_seqs(reviews, True, True)
            batch_dec_inps.append(batch_lines)
            batched_data.append((batch_keys, batch_reviews, batch_dec_inps))


        return batched_data


    def build_review_batches(self, ori_data, batch_size, mode):
        ...
        for bi in range(0, batch_num):
            ...
            reviews = [instance[1] for instance in instances]  # all reviews
            # sequences = [sum(rev, []) for rev in reviews]
            batch_reviews = self.__build_batch_seqs(reviews, True, True)
