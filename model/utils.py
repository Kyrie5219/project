...
def sample_dae(ori_keys, ori_reviews, all_trgs, all_logits, sample_num, tool):
    # enc_inps [batch size, sen len]
    # all_logits [batch size, trg len, vocab size] * sens_num
    batch_size = ori_reviews.size(0)
    enc_len = ori_reviews.size(1)
    ...
    for sid in sample_ids:
        # Build lines
        inps = [ori_reviews[sid, t].item() for t in range(0, enc_len)]
